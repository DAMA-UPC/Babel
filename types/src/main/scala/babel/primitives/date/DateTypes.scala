...
/**
  * Trait containing all [[DateTypes]]s to Babel [[babel.Type]]s implicit conversions.
  */
trait DateTypes {
  ...
  private[date] val astDateType: DateType =
    DateType(DateTypeConstraints.minDate(defaultDateOnAST), DateTypeConstraints.maxDate(babel.Now))
...
object DateTypes extends DateTypes {
  ...
  private[babel] def typeNameToBabelType(typeName: String): Option[DateType] = {
